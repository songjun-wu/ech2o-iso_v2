...
int Basin::SolveSurfaceFluxes(Atmosphere &atm, Control &ctrl, Tracking &trck) {
  ...
  double leak = 0; //bedrock leakage flux [m s-1]
  double leakdepth = 0; //bedrock leakage depth [m]
...
#pragma omp for nowait
    ...
    //update leakage depth for tracer mixing
    leakdepth = leak*dt;
	// Tracking
	if(ctrl.sw_trck)
	  trck.MixingV_down(*this, ctrl, d1, d2, d3, fc, leakdepth, r, c, 0);
	...
	_BedrockLeakageDepth->matrix[r][c] = leakdepth; // m
