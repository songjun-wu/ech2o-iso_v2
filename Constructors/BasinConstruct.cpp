...
Basin::Basin(Control &ctrl)
{
  try{
    ...
    //2022 surface overland flow mixing ratio 
    if(ctrl.toggle_SrfOVFmix ==1) {	
      _Srfmixratio = new grid(ctrl.path_BasinFolder + ctrl.fn_ratiosrfmix, ctrl.MapType);
    }
    ...
    _BedrockLeakageDepth = new grid(*_DEM); //bedrock leakage depth in m 
    ...
    _FluxSrftoL1_r = new grid(*_DEM); // infiltration from first layer
    _FluxInfilt_r = new grid(*_DEM); // timestep-summed infiltration from first layer
    _FluxL1toL2_r= new grid(*_DEM); // percolation L1 to L2
    _FluxL2toL3_r= new grid(*_DEM); // percolation L2 to L3
    _Transpiration_L1= new grid(*_DEM); //transpiration from canopy in m s-1
    _Transpiration_L2= new grid(*_DEM); //transpiration from canopy in m s-1
    _Transpiration_L3= new grid(*_DEM); //transpiration from canopy in m s-1
    _QLattoChn= new grid(*_DEM); //_FluxLattoChn in m^3/s
    _QChntoLat= new grid(*_DEM); //_FluxChntoLat in m^3/s
    _QChanstore= new grid(*_DEM); //_FluxChntoLat in m^3/s
    // Accumulated fluxes
	...
	delete _BedrockLeakageFlux;
      if(_BedrockLeakageDepth)
	delete _BedrockLeakageDepth;
      if(_SoilWaterDepth)
	...
	delete _Transpiration_all;
      if(_Transpiration_L1)
	delete _Transpiration_L1;
      if(_Transpiration_L2)
	delete _Transpiration_L2;
      if(_Transpiration_L3)
	delete _Transpiration_L3;
      if(_Temp_w)
	delete _Temp_w;
      if(_FTemp_w)
	delete _FTemp_w;
	...
	delete _FluxInfilt;
      if(_FluxSrftoL1_r)
	delete _FluxSrftoL1_r;
      if(_FluxInfilt_r)
	delete _FluxInfilt_r;
      if(_FluxExfilt)
	delete _FluxExfilt;
      if(_FluxRecharge)
	...
	delete _FluxL2toL3;
      if(_FluxL1toL2_r)
	delete _FluxL1toL2_r;
      if(_FluxL2toL3_r)
	delete _FluxL2toL3_r;
      //if(_FluxL2toGW)
      //		delete _FluxL2toGW;
      //if(_FluxL3toGW)
      //	delete _FluxL3toGW;
