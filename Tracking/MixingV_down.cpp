			    ...
			    int step) // 0 for SolveSurface, 1 for GWrouting
{
  ...
  //re-infil
  double SrftoL1_r = bsn.getFluxSrftoL1R()->matrix[r][c];
  double L1toL2_r = bsn.getFluxL1toL2R()->matrix[r][c];
  double L2toL3_r = bsn.getFluxL2toL3R()->matrix[r][c];
  ...
  if(ctrl.sw_TPD){
    ...
    // Routing to tightly-bound domain: fraction from relative volume of "inactive"-pore domain, 
    // limited by the "available space" in tightly-bound domain max(0,d1*(theta_MW-theta1_old))
      SrftoTB1 = std::min<double>(SrftoL1_r,//*(theta_MW-theta_r)/(porosity-theta_r),
				std::max<double>(0,d1*(theta_MW1-theta1_old)));
    // Remainder directly goes to mobile water
      SrftoMW1 = std::max<double>(0,SrftoL1_r-SrftoTB1);
    // L2: Percolation only from mobile water in L1
      MW1toTB2 = std::min<double>(L1toL2_r,//*(theta_MW-theta_r)/(porosity-theta_r),
				std::max<double>(0,d2*(theta_MW2-theta2_old)));
      MW1toMW2 = std::max<double>(0,L1toL2_r-MW1toTB2);
    // L3: Percolation only from mobile water water in L2 (MW2toL3 = L2toL3). 
    }
  }
  ...
  if(step == 1){
	...
	_d2Hsurface->matrix[r][c] = pond_old > RNDOFFERR ? 
	  InOutMix(pond_old, _d2Hsurface->matrix[r][c], FinSrf, d2Hin, SrftoL1_r, mixmod) : d2Hin ;
	...
	_d18Osurface->matrix[r][c] = pond_old > RNDOFFERR ? 
	  InOutMix(pond_old, _d18Osurface->matrix[r][c], FinSrf, d18Oin, SrftoL1_r, mixmod) : d18Oin;
	...
	_Agesurface->matrix[r][c] = pond_old > RNDOFFERR ? 
	  InOutMix(pond_old, _Agesurface->matrix[r][c], FinSrf, Agein, SrftoL1_r, mixmod) : Agein ;
      }
    }      
  }
  ...
  if(step == 0 ){
	...
	_AgeRecharge->matrix[r][c] =_Age_MW2->matrix[r][c] ;
      }
	...
	_AgeRecharge->matrix[r][c] = _Agesoil2->matrix[r][c];
      }
    }
  ...
  if(step==1 and ctrl.sw_reinfilt){
    // Layer 1 ------------------------------------------------------------------------
   if (SrftoL1_r>RNDOFFERR){
    // If two-pore domain activated, and different groundwater origin (MW2 instead of soil2)
    if(ctrl.sw_TPD){
      if(ctrl.sw_2H){
	 d_old = _d2H_TB1->matrix[r][c];
	 ...
	 _d2H_MW1->matrix[r][c] = std::max<double>(0,theta1_old-theta_MW1) > RNDOFFERR ?
	  InOutMix(std::max<double>(0,theta1_old-theta_MW1)*d1, _d2H_MW1->matrix[r][c], 
		   SrftoMW1, _d2Hsurface->matrix[r][c], L1toL2_r, mixmod) : 
	  _d2Hsurface->matrix[r][c] ;
	 ...
	 if(abs(_d2H_MW1->matrix[r][c])>300) // or (r==40 and c==92))
	  cout << r << " " << c << "| d2H " << 
	    "| dMW1_new:" << _d2H_MW1->matrix[r][c] << "| dMW1_old:" << d_old << 
	    "| dSrf:" << _d2Hsurface->matrix[r][c] <<
	    "| SrftoMW1:" << SrftoMW1 << "| MW1toL2:" << L1toL2_r << endl;
      }
	...
	if(abs(_d18O_TB1->matrix[r][c])>100)
	 ...
	 _d18O_MW1->matrix[r][c] = std::max<double>(0,theta1_old-theta_MW1) > RNDOFFERR ?
	  InOutMix(std::max<double>(0,theta1_old-theta_MW1)*d1, _d18O_MW1->matrix[r][c], 
		   SrftoMW1, _d18Osurface->matrix[r][c], L1toL2_r, mixmod) : 
	  _d18Osurface->matrix[r][c] ;
	 ...
	 _Age_MW1->matrix[r][c] = std::max<double>(0,theta1_old-theta_MW1) > RNDOFFERR ?
	  InOutMix(std::max<double>(0,theta1_old-theta_MW1)*d1, _Age_MW1->matrix[r][c], 
		   SrftoMW1, _Agesurface->matrix[r][c], L1toL2_r, mixmod) : 
	  _Agesurface->matrix[r][c] ;
      ...
      if(ctrl.sw_2H)
	 _d2Hsoil1->matrix[r][c] = InOutMix(theta1_old*d1, _d2Hsoil1->matrix[r][c],
					   SrftoL1_r, _d2Hsurface->matrix[r][c], L1toL2_r, mixmod);
    
      if(ctrl.sw_18O)
	 _d18Osoil1->matrix[r][c] = InOutMix(theta1_old*d1, _d18Osoil1->matrix[r][c],
					    SrftoL1_r, _d18Osurface->matrix[r][c], L1toL2_r, mixmod);
    
      if(ctrl.sw_Age)
	 _Agesoil1->matrix[r][c] = InOutMix(theta1_old*d1, _Agesoil1->matrix[r][c],
					   SrftoL1_r, _Agesurface->matrix[r][c], L1toL2_r, mixmod);
    ...
    // If two-pore domain activated: only MW1 percolates
    if(ctrl.sw_TPD and L1toL2_r>RNDOFFERR){
      if(ctrl.sw_2H){
	_d2H_TB2->matrix[r][c] = InputMix(std::min<double>(theta2_old,theta_MW2)*d2, 
					  _d2H_TB2->matrix[r][c],
					  MW1toTB2, _d2H_MW1->matrix[r][c]);
	_d2H_MW2->matrix[r][c] = std::max<double>(0,theta2_old-theta_MW2) > RNDOFFERR ?
	  InOutMix(std::max<double>(0,theta2_old-theta_MW2)*d2, _d2H_MW2->matrix[r][c], 
		   MW1toMW2, _d2H_MW1->matrix[r][c], L2toL3_r, mixmod) : _d2H_MW1->matrix[r][c] ;
	...
	_d18O_MW2->matrix[r][c] = std::max<double>(0,theta2_old-theta_MW2) > RNDOFFERR ?
	  InOutMix(std::max<double>(0,theta2_old-theta_MW2)*d2, _d18O_MW2->matrix[r][c], 
		   MW1toMW2, _d18O_MW1->matrix[r][c], L2toL3_r, mixmod) : _d18O_MW1->matrix[r][c] ;
	...
	_Age_MW2->matrix[r][c] = std::max<double>(0,theta2_old-theta_MW2) > RNDOFFERR ?
	  InOutMix(std::max<double>(0,theta2_old-theta_MW2)*d2, _Age_MW2->matrix[r][c], 
		   MW1toMW2, _Age_MW1->matrix[r][c], L2toL3_r, mixmod) : _Age_MW1->matrix[r][c] ;	
      }
    } else if (L1toL2_r > RNDOFFERR){
      if(ctrl.sw_2H)
	_d2Hsoil2->matrix[r][c] = InOutMix(theta2_old*d2, _d2Hsoil2->matrix[r][c],
					   L1toL2_r, _d2Hsoil1->matrix[r][c], L2toL3_r, mixmod);
    
      if(ctrl.sw_18O)
	_d18Osoil2->matrix[r][c] = InOutMix(theta2_old*d2, _d18Osoil2->matrix[r][c],
					    L1toL2_r, _d18Osoil1->matrix[r][c], L2toL3_r, mixmod);
    
      if(ctrl.sw_Age)
	_Agesoil2->matrix[r][c] = InOutMix(theta2_old*d2, _Agesoil2->matrix[r][c],
					   L1toL2_r, _Agesoil1->matrix[r][c], L2toL3_r, mixmod);
    ...
    // If two-pore domain activated: only MW2 percolates
    // note that there are no further leakage during the reinfiltration!!
    if(ctrl.sw_TPD and L2toL3_r>RNDOFFERR){
      if(ctrl.sw_2H)
	_d2Hsoil3->matrix[r][c] = InputMix(theta3_old*d3, _d2Hsoil3->matrix[r][c],
					   L2toL3_r, _d2H_MW2->matrix[r][c]);
      
      if(ctrl.sw_18O)
	_d18Osoil3->matrix[r][c] = InputMix(theta3_old*d3, _d18Osoil3->matrix[r][c],
					    L2toL3_r, _d18O_MW2->matrix[r][c]);

      if(ctrl.sw_Age){
	_Agesoil3->matrix[r][c] = InputMix(theta3_old*d3, _Agesoil3->matrix[r][c], 
					   L2toL3_r, _Age_MW2->matrix[r][c]);
	_AgeRecharge->matrix[r][c] = InputMix(std::max<double>(0.0,bsn.getFluxRecharge()->matrix[r][c] - L2toL3_r), 
		   _AgeRecharge->matrix[r][c], L2toL3_r, _Age_MW2->matrix[r][c]);
      }
      
    } else if (L2toL3_r>RNDOFFERR){
      if(ctrl.sw_2H)
	_d2Hsoil3->matrix[r][c] = InputMix(theta3_old*d3, _d2Hsoil3->matrix[r][c],
					   L2toL3_r, _d2Hsoil2->matrix[r][c]);
      
      if(ctrl.sw_18O)
	_d18Osoil3->matrix[r][c] = InputMix(theta3_old*d3, _d18Osoil3->matrix[r][c],
					    L2toL3_r, _d18Osoil2->matrix[r][c]);
      
      if(ctrl.sw_Age){
	_Agesoil3->matrix[r][c] = InputMix(theta3_old*d3, _Agesoil3->matrix[r][c],
					   L2toL3_r, _Agesoil2->matrix[r][c]);
	_AgeRecharge->matrix[r][c] = InputMix(std::max<double>(0.0,bsn.getFluxRecharge()->matrix[r][c] - L2toL3_r), 
		   _AgeRecharge->matrix[r][c], L2toL3_r, _Agesoil2->matrix[r][c]);
      }
    }
  ...
  if(step == 0 ){ // step ==1 no further leakage into deeper groundwater 
