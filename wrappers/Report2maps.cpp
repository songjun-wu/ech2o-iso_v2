...
int Report2Maps(){
  ...
  if(oControl->sw_trck && oControl->sw_2H){
    ...
    if (oControl->Rep_d2HSrftoChn)
      WriteMapSeries(oTracking->getd2HSrftoChn(), "dHSrfQ", oControl->current_ts_count);
  ...
  if(oControl->sw_trck && oControl->sw_18O){
    ...
    if (oControl->Rep_d18OSrftoChn)
      WriteMapSeries(oTracking->getd18OSrftoChn(), "dOSrfQ", oControl->current_ts_count);
...
int Report2Ts(){
  ...
  if(oControl->sw_trck && oControl->sw_2H){
    ...
    if (oControl->RepTs_d2HSrftoChn){
      if(oControl->GetTimeStep() <= oControl->report_times)
	oReport->RenameFile(oControl->path_ResultsFolder + "d2H_SrftoChn.tab");
      oReport->ReportTimeSeries(oTracking->getd2HSrftoChn(),
				oControl->path_ResultsFolder + "d2H_SrftoChn.tab",
				oControl->current_ts_count);
    }
  ...
  if(oControl->sw_trck && oControl->sw_18O){
    ...
    if (oControl->RepTs_d18OSrftoChn){
      if(oControl->GetTimeStep() <= oControl->report_times)
	oReport->RenameFile(oControl->path_ResultsFolder + "d18O_SrftoChn.tab");
      oReport->ReportTimeSeries(oTracking->getd18OSrftoChn(),
				oControl->path_ResultsFolder + "d18O_SrftoChn.tab",
				oControl->current_ts_count);
    }
