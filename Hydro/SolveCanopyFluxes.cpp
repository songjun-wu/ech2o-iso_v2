...
int Basin::SolveCanopyFluxes(Atmosphere &atm, Control &ctrl, Tracking &trck) {
  ...
  REAL8 transp_f1 = 0; //transpiration for the entire cell L1
  REAL8 transp_f2 = 0; //transpiration for the entire cell L2
  REAL8 transp_f3 = 0; //transpiration for the entire cell L3
  //REAL8 ETP;
  ...
  REAL8 wiltingpoint; //get for each species
  REAL8 wiltingpoint_f; //overall mean for the grid
...
#pragma omp parallel default(none)					\
  private( s, r,c, p,  treeheight, wind, za, z0o, zdo,			\
	   Tp, maxTp, minTp, snow, rain, sno_rain_thres, evap,wiltingpoint,wiltingpoint_f,\
	   transp, netR, evap_f, transp_f, transp_f1,transp_f2,transp_f3, \
       D, DelCanStor, theta, theta2, theta3, theta_available, ra, \
...
#pragma omp for nowait
	...
	if (s == nsp - 1) { //if this is bare ground set D to precip and skip the tree stuff
	  ...
	  fForest->SolveCanopyEnergyBalance(*this, atm, ctrl, 
					    ...
					    ra, DelCanStor, evap, wiltingpoint, transp, netR, 
					    s, r, c);
      ...
      wiltingpoint_f += wiltingpoint * p; //wiltingpoint
	  // ---------------------------
	  if(ctrl.sw_trck){
	    if(ctrl.sw_2H)
	  ...
	  transp_f += transp * p;
	  //yangx 022-01
	  transp_f1 += transp * p * froot1;
	  transp_f2 += transp * p * froot2;
	  transp_f3 += transp * p * froot3;
	...
	_ponding->matrix[r][c] += rain * dt * p;
      ...
      _Transpiration_L1->matrix[r][c]  = transp_f1 ;
      _Transpiration_L2->matrix[r][c]  = transp_f2 ;
      _Transpiration_L3->matrix[r][c]  = transp_f3 ;
	  _Wiltingpnt->matrix[r][c]  = wiltingpoint_f/nsp;  //get the grid average value
