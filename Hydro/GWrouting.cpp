...
int Basin::DailyGWRouting(Atmosphere &atm, Control &ctrl, Tracking &trck) {
  ...
  if(ctrl.sw_chan_evap)
	...
	for (unsigned int j = 0; j < _vSortedGrid.cells.size(); j++) {
      ...
      leak_depth = _BedrockLeakageDepth->matrix[r][c];
      ...
      if (ctrl.sw_reinfilt){
		...
		//explicit reinfiltration flux
	    _FluxInfilt_r->matrix[r][c] = _FluxSrftoL1_r->matrix[r][c];
	    ...
	    // Back up ponding before GW seepage (for rivers)
	      _ponding->matrix[r][c] = Rponding;

	  } else { //equivalent to the original settings with reinfiltration being turned off
          Rponding = ponding ;
		  srfrnoff = ponding ;
     	// Tracking
	    if(ctrl.sw_trck){
	    // Tracer mixing accounting for snowmelt and lateral input
	  ...
	  if (ctrl.sw_channel && _channelwidth->matrix[r][c] > 0) {
	    ...
	    _FluxSrftoChn->matrix[r][c] = srfrnoff; //[m]
	  ...
	  switch (d) 
	    ...
	    case 5: //if it is an outlet store the outflow m3s-1
          ...
          Outletpond = Rponding*actArea/ dt; //[m3/s]directly put into final outputs
	  ...
	  if(lat_ok){
	    ...
	    _FluxLattoSrf->matrix[rr][cc] += Rponding ;
	    _FluxLattoChn->matrix[rr][cc] += Qk1*dtdx/_dx;
        _QLattoChn->matrix[rr][cc]  += Qk1; // [m3/s]
	    _FluxLattoGW->matrix[rr][cc] += hj1i1 * alpha * dtdx;
	    // Accumulated fluxes
	    _AccLattoSrf->matrix[rr][cc] += Rponding ; //ponding
	    ...
	    _ponding->matrix[rr][cc] += Rponding ;     	

	  }

	  // Outgoing water (outside of lat_ok because can be 0)
	  //if(ctrl.sw_trck){
	  _FluxSrftoLat->matrix[r][c] = lat_ok != 0 ? Rponding : 0.0;
      ...
      _QChntoLat->matrix[r][c] = lat_ok != 0 ? Qk1 : 0.0;	//[m3/s]
	  // Accumulated fluxes
	  ...
	  if(ctrl.sw_trck){
	    if (!ctrl.sw_extraGW){
		  trck.MixingV_latup(*this, ctrl, d1, d2, d3, fc, 
			         Qk1, dtdx, _dx, r, c);
          //update soil moisture after lat & up fluxes
	      _soilmoist1->matrix[r][c] = theta1;
	      _soilmoist2->matrix[r][c] = theta2;
	      _soilmoist3->matrix[r][c] = theta3 + hj1i1 / d3;
		  //update tracer concentrations of all runoff components seepaging into channels
		  if(ctrl.sw_channel && _channellength->matrix[r][c] > 0){
		  trck.MixingChannel_CRunoff(*this, ctrl,poros1, r, c);
		  }
	      ...
	      trck.MixingV_latupDGW(*this, ctrl, d1, d2, d3, fc,Qk1, 
		                          dtdx, _dx, dt, r, c);
          //update soil moisture after lat & up fluxes
	      _soilmoist1->matrix[r][c] = theta1;
	      _soilmoist2->matrix[r][c] = theta2;
		  ...
		  //update tracer concentrations of all runoff components seepaging into channels
		  if(ctrl.sw_channel && _channellength->matrix[r][c] > 0){
		  trck.MixingChannel_CRunoffDGW(*this, ctrl,poros1, r, c);
		  }
		  if(lat_ok == 1){
			trck.FCdownstream_DGW(*this, ctrl, Qk1, dtdx, _dx, r, c, rr, cc);
		  }else{
			trck.OutletVals_DGW(*this, ctrl, 1, Qk1, Outletpond, r, c);
		  }
		}
	  }
	  ...
	  if (ctrl.sw_channel && _channelwidth->matrix[r][c] > 0){
        ...
        //only for coupling purpose in m^3 --yangx 2022-02
		_QChanstore->matrix[r][c] = Si1j1;
        if (ctrl.sw_chan_evap){
